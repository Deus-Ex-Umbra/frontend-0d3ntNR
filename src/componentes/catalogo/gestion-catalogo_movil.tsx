...
interface GestionCatalogoMovilProps {
  titulo: string;
  items: ItemCatalogo[];
  cargando: boolean;
...
export function GestionCatalogoMovil({
  titulo,
  items,
  cargando,
  onCrear,
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base md:text-lg font-semibold text-foreground">{titulo}</h3>
            <p className="text-xs md:text-sm text-muted-foreground">{items.length} registros</p>
          </div>
        </div>
        <Button
          onClick={() => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          {item_a_eliminar && (
            <div className="p-3 md:p-4 rounded-lg bg-secondary/30 border border-border">
              <p className="font-semibold text-foreground text-sm md:text-base">"{item_a_eliminar.nombre}"</p>
              <p className="text-xs md:text-sm text-muted-foreground mt-1">{titulo}</p>
            </div>
          )}
      ...
      <Dialog open={dialogo_crear_abierto} onOpenChange={setDialogoCrearAbierto}>
        <DialogContent className="sm:max-w-[500px]">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-3">
                  <Input
                    ...
                    className="h-9 md:h-10 w-16 md:w-20 cursor-pointer hover:scale-105 transition-all duration-200"
                  />
                  <Input
                    type="text"
                    value={formulario.color}
          ...
          <DialogFooter className="flex-col sm:flex-row">
            ...
            <Button onClick={manejarCrear} disabled={guardando} className="w-full sm:w-auto">
      ...
      <Dialog open={dialogo_editar_abierto} onOpenChange={setDialogoEditarAbierto}>
        <DialogContent className="sm:max-w-[500px]">
          ...
          <DialogFooter className="flex-col sm:flex-row">
            ...
            <Button onClick={manejarEditar} disabled={guardando} className="w-full sm:w-auto">
              {guardando && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
              Actualizar
            </Button>
          </DialogFooter>
